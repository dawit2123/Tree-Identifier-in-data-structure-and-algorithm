...
//node down to the farthest leaf node of the tree. 
   ...
   if(abs(lh-rh) <= 1 && checkAVL(root->left) && checkAVL(root->right)) return 1;
   return 0;
}
/*#################################################################HEAP####3*/
...
void menu()
{
    ...
    if (choice)
        ...
        cout<<"Do you want ot continue again(Y/N)"<<endl;
  cin>>test;
}
while(test=='Y'||test=='y');
        }
...
void onlyBST(node* root){
 ...
 void all(node* root){
 if(checkBST(root)){
 	if(checkBST(root)||checkAVL(root)||checkRedblack(root)||checkHeap(root)){
	 
      cout<<"Given binary tree is a BST \n";
 	cout<<"----------------------------------\n";}
 if(checkAVL(root)){
      cout<<"Given binary tree is a AVL \n";
 	cout<<"----------------------------------\n";}
 if(checkRedblack(root)){
      cout<<"Given binary tree is a RedBlack \n";
 	cout<<"----------------------------------\n";}
 if(checkHeap(root)){
        cout<<"Given binary tree is a HEAP \n";  
 	cout<<"----------------------------------\n";}
 	    ...
 	    cout<<"the given binary tree is not BST,AVL, RED-BLACK, HEAP\n";
 }